...
// Must fit the real datatype:
#define MY_MPI_REAL MPI_DOUBLE         

const int NDIM = 3;                        // number of spatial dimensions

#define IDX(i,j) 	[i*NDIM + j]

void correct_step(real *pos, real *vel, 
                  const real *acc, const real *jerk,
                  const real *old_pos, const real *old_vel, 
                  const real *old_acc, const real *old_jerk,
                  int n, real dt);
void evolve(const real mass[], real *pos, real *vel,
            int n, real & t, real dt_param, real dt_dia, real dt_out,
            real dt_tot, bool init_out, bool x_flag);
void evolve_step(const real mass[], real *pos, real *vel,
                 real *acc, real *jerk, int n, real & t,
                 real dt, real & epot, real & coll_time);
void get_acc_jerk_pot_coll(const real mass[], const real *pos,
                           const real *vel, real *acc,
                           real *jerk, int n, real & epot,
                           real & coll_time);
void get_snapshot(real mass[], real *pos, real *vel, int n);
void predict_step(real *pos, real *vel, 
                  const real *acc, const real *jerk,
                  int n, real dt);
void put_snapshot(const real mass[], const real *pos,
                  const real *vel, int n, real t);
bool read_options(int argc, char *argv[], real & dt_param, real & dt_dia,
                  real & dt_out, real & dt_tot, int & i_flag, int & x_flag);
void write_diagnostics(const real mass[], const real *pos,
                       const real *vel, const real *acc,
                       const real *jerk, int n, real t, real epot,
...
int main(int argc, char *argv[])
{
  ...
  // All processes run in the same directory, 
  // so give profile output files distinct names.
  MPI_Get_processor_name(nodename, &nodename_len);
  nodename[nodename_len] = 0;
  setenv("GMON_OUT_PREFIX", nodename, 1);
  ...
  int  init_out = 0;    // if true: snapshot output with start at t = 0
  //          with an echo of the input snapshot
  int  x_flag = 0;      // if true: extra debugging diagnostics output

  int keepGoing = 0;

  if (0==myRank)
    keepGoing = read_options(argc, argv, dt_param, dt_dia, dt_out, dt_tot, init_out, x_flag);

  MPI_Bcast(&keepGoing, 1, MPI_INT, 0, comm);
  if(!keepGoing) {
    MPI_Finalize();
  ...
  MPI_Bcast(&dt_param, 1, MY_MPI_REAL, 0, comm);
  MPI_Bcast(&dt_dia, 1, MY_MPI_REAL, 0, comm);
  MPI_Bcast(&dt_out, 1, MY_MPI_REAL, 0, comm);
  MPI_Bcast(&dt_tot, 1, MY_MPI_REAL, 0, comm);
  MPI_Bcast(&init_out, 1, MPI_INT, 0, comm);
  MPI_Bcast(&x_flag, 1, MPI_INT, 0, comm);
  ...
  if(0==myRank)
    cin >> t;
  MPI_Bcast(&t, 1, MY_MPI_REAL, 0, comm);

  real * mass = new real[n];          // masses for all particles
  real * pos  = new real[n*NDIM];     // positions for all particles
  real * vel  = new real[n*NDIM];     // velocities for all particles

  if(0==myRank)
    get_snapshot(mass, pos, vel, n);

  MPI_Bcast(mass, n, MY_MPI_REAL, 0, comm);
  MPI_Bcast(pos, n*NDIM, MY_MPI_REAL, 0, comm);
  MPI_Bcast(vel, n*NDIM, MY_MPI_REAL, 0, comm);
...
void get_snapshot(real mass[], real *pos, real *vel, int n)
{
  for (int i = 0; i < n ; i++){
    ...
    for (int k = 0; k < NDIM; k++)
      cin >> pos IDX(i,k);                 // position of particle i
    for (int k = 0; k < NDIM; k++)
      cin >> vel IDX(i,k);                 // velocity of particle i
  }
}
                  ...
                  const real *vel, int n, real t)
{
  ...
  for (int i = 0; i < n ; i++){
    ...
    for (int k = 0; k < NDIM; k++)
      cout << ' ' << pos IDX(i,k);         // position of particle i
    for (int k = 0; k < NDIM; k++)
      cout << ' ' << vel IDX(i,k);         // velocity of particle i
    cout << endl;
  }
}
                       ...
                       bool x_flag)
{
  ...
  for (int i = 0; i < n ; i++)
    for (int k = 0; k < NDIM ; k++)
      ekin += 0.5 * mass[i] * vel IDX(i,k) * vel IDX(i,k);
            ...
            real dt_tot, bool init_out, bool x_flag)
{
  ...
  real * acc  = new real[n*NDIM];          // accelerations and jerks
  real * jerk = new real[n*NDIM];         // for all particles
...
void evolve_step(const real mass[], real *pos, real *vel,
                 ...
                 real dt, real & epot, real & coll_time)
{
  static real *old_pos  = new real[n*NDIM];
  static real *old_vel  = new real[n*NDIM];
  static real *old_acc  = new real[n*NDIM];
  static real *old_jerk = new real[n*NDIM];
  ...
  memcpy(old_pos,  pos,  n*NDIM*sizeof(real));
  memcpy(old_vel,  vel,  n*NDIM*sizeof(real));
  memcpy(old_acc,  acc,  n*NDIM*sizeof(real));
  memcpy(old_jerk, jerk, n*NDIM*sizeof(real));
...
{
  real dtdt_2 = dt*dt/2;
  real dtdtdt_6 = dt*dt*dt/6;

  for (int i = 0; i < n; i++)
    for (int k = 0; k < NDIM ; k++){
      pos IDX(i,k) += vel IDX(i,k) *dt + acc IDX(i,k) *dtdt_2 + jerk IDX(i,k) *dtdtdt_6;
      vel IDX(i,k) += acc IDX(i,k) *dt + jerk IDX(i,k) *dtdt_2;
    }
}
...
{
  real dt2 = 0.5 * dt;
  real dtdt12 = dt*dt/12;

  for (int i = 0; i < n; i++)
    for (int k = 0; k < NDIM ; k++) {
      vel IDX(i,k) = old_vel IDX(i,k) + (old_acc IDX(i,k) + acc IDX(i,k))*dt2
	+ (old_jerk IDX(i,k) - jerk IDX(i,k))*dtdt12;
      pos IDX(i,k) = old_pos IDX(i,k) + (old_vel IDX(i,k) + vel IDX(i,k))*dt2
	+ (old_acc IDX(i,k) - acc IDX(i,k))*dtdt12;
    }
}
...
void get_acc_jerk_pot_coll(const real mass[], const real *pos,
                           ...
                           real & coll_time)
{
  bzero(acc, n*NDIM*sizeof(real));
  bzero(jerk,n*NDIM*sizeof(real));
  
  epot = 0;
  const real VERY_LARGE_NUMBER = 1e300;
  real coll_time_q = VERY_LARGE_NUMBER;      // collision time to 4th power
  real coll_est_q; // collision time scale estimate to 4th power (quartic)
  ...
  for (int i = begin; i < end ; i++){
	...
	continue;
      ...
      for (int k = 0; k < NDIM ; k++){
	rji[k] = pos IDX(j,k) - pos IDX(i,k);
	vji[k] = vel IDX(j,k) - vel IDX(i,k);
      }
      real r2 = 0;                           // | rji |^2
      real v2 = 0;                           // | vji |^2
	...
	dj[k] = (vji[k] - 3 * rv_r2 * rji[k]) / r3;    // in the header
      ...
      for (int k = 0; k < NDIM ; k++){
	acc IDX(i,k) += mass[j] * da[k];                 // using symmetry
	//acc IDX(j,k) -= mass[i] * da[k];                 // find pairwise
	jerk IDX(i,k) += mass[j] * dj[k];                // acceleration
	//jerk IDX(j,k) -= mass[i] * dj[k];                // and jerk
      }
  ...
  MPI_Allreduce(MPI_IN_PLACE, &coll_time_q,      1, MY_MPI_REAL, MPI_MIN, comm);
  MPI_Allreduce(MPI_IN_PLACE, &epot       ,      1, MY_MPI_REAL, MPI_SUM, comm);
  MPI_Allgather(MPI_IN_PLACE,-1,MY_MPI_REAL,
		acc, (end-begin)*NDIM,MY_MPI_REAL,
		comm);
  MPI_Allgather(MPI_IN_PLACE,-1,MY_MPI_REAL,
		jerk,(end-begin)*NDIM,MY_MPI_REAL,
		comm);
  double tock = MPI_Wtime();
  static double elapsed = 0;
  elapsed += tock - tick;
  fprintf(stderr, "[%d] get_acc*: %g s in MPI\r", myRank, elapsed);

  // from q for quartic back to linear collision time
